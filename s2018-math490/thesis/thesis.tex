...
$f(x) = v$ should be roughly on the order of $2^{b}$. Given such an attack,
...
\begin{align*}
    C[x, z] & := \oplus_{0 \leq y \leq 4} A[x, y, z] \\
...
$0 \leq z < w: A'[0, 0, z] := A[0, 0, z]$. Then let $x, y = (0, 0)$, and for
$0 \leq t \leq 23$:
\begin{align*}
    \forall 0 \leq z < w: A'[x, y, z] & := A[x, y, (z - \frac{(t+1)(t+2)}{2}) \text{ mod } w] \\
    let x, y & = (y, (2x + 3y) \text{ mod } 5)
\end{align*}
...
\end{align*}
    ...
    $\chi$ is the only non-linear function and is of degree one (with inverse

...
best second preimage attack by H. Yu et al. gives a bound of $2^{-56}$
...
collisions in MD4 from collisions in reduced-round MD4 and to build addition
...
new collision class, we can easily split our search to look in each of the
...
$F_1$ and $F_2$ are \textit{similar}, and notate it $F_1 \lesssim F_2$, if
...
From an intuitive standpoint, what this is says is that collisions have
witnesses for the existence in reduced round versions of the function, and
...
likelihood. That is, we can transplant a $n$ round collision into a $n+4$
...
works, but requires searching wide and wider differential paths in small round
...
\begin{itemize}
    ...
    \item Keeping small search radius on neighborhoods involving families
        ...
        families can be found by various filling-in techniques by applying
        the $\lesssim$ relation.
\end{itemize}
...
including the specified one, which produce a differential path conforming to
...
same number of differentials as thus is also more flexible than Y. Sasaki's.
...
attack--have a larger neighborhood sizes, and likewise with P. Kasselman's
...
neighborhood search does not lead to collisions in every case. Therefore,

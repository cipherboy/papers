...
difficult; one-way - it should be infeasible to generate a message
from its hash except by trying all possible messages; sensitive to
...
The most important aspect of these can be formally defined as three
properties which hash functions must have to be considered cryptographically secure:
\begin{itemize}
...
\emdash faster than the theoretical $2^{80}$ but still requiring significant compute
resources to find.
...
\begin{itemize}
    ...
    \item Keeping a small search radius on neighborhoods involving families
        ...
        techniques through applying the $\lesssim$ relation.
\end{itemize}

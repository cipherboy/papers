...
\title{Measuring the Utility of a Collision:\\ The Neighborhood of MD4 Collisions}
...
% in the abstract
\begin{abstract}
We propose a series of metrics for evaluating the utility of collision classes
and develop a set of techniques for generating new collision classes in hash
functions utilizing a logical cryptanalaysis framework. We demonstrate the
effectiveness of this framework on the hash function MD4 and show that the
existing attacks are of high utility, but do not allow for second preimage
attacks under an expansive neighborhood. We partially analyze over 35,000 new
collision classes for their utility under these methods.
\end{abstract}
...
We introduce a distance function between collision classes by the number of
differences in intermediate rounds deltas. We find justification for this
metric in the existing literature on MD4: H. Dobbertin's \cite{Dobbertin1998}
and P. Kasselman's \cite{KasselmanMD4} classes have distance $1$ under this
metric.

That is:
\begin{align}
    \delta(C_1, C_2) = \| \{ i : I(C_1, i) \neq I(C_2, i) \} \|
\end{align}

For the example collisions, below are distances under this metric:
\textbf{TODO}.


\subsection{Neighborhood}

We define the neighborhood of a collision class to be all other collisions
which have a distance of one.

That is:
\begin{align}
    N(C) = \{ C_j : \delta(C, C_j) = 1 \}
\end{align}


For the example collisions, below are sizes of the neighborhoods:
\textbf{TODO}.
...
Include graphic of count of unique differentials for all 35k collision classes

Separate table for known attacks as data points. Examples in appendix?

\subsection{Unit-Step Neighborhood}

\textbf{TODO}

Include graphic of count of number of neighbors for all 35k collision classes

Separate table for known attacks as data points.

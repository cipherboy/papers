...
into as $\mathcal{V}$. We define a set of functions $R(x)$ where $x$
is an element derived from the sets $\mathcal{C}$ or $\mathcal{F}$
...
We seek to evaluate trustworthiness by building a measure of the relative difference between a collision
...
We find justification for the distance heuristic, $\delta$, defined in Section
...
under this metric. Since others have shown that MD4 is untrustworthy, we can
...
similar differential paths. Further, we can achieve parallel solving
capabilities by noting that this problem is embarrassingly parallel: each
candidate collision class can be independently checked on separate machines.
